...
for (( i=0; i<${n_analyses}; i++)); do
    ...
    Rscript phenorm_covid/03_get_results.R --data-dir "${analysis_data_dir}" --output-dir "${output_dir}" --analysis "$analysis" --weight ${weight_var} --data-site "${site}" --model-site "${external_site}" --study-id ${study_id} > "./${io_dir}/03_get_results_${analysis}_${site}_${external_site}.out" 2>&1
