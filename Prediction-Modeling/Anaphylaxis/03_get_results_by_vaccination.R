...
# make sure that analysis_data$data colnames are all uppercase
analysis_data$data <- analysis_data$data %>% 
  rename_with(toupper)
...
# evaluate performance separately for those with vaccine receipt vs those without
perf_names <- names(preds)
vaccine_values <- exp(unique(analysis_data$full$vaccine)) - 1
for (v in 1:length(vaccine_values)) {
  this_value <- vaccine_values[v]
  these_participants <- analysis_data$data$VACCINE[analysis_data$test_ids] == log(this_value + 1)
  
  perf_list <- lapply(as.list(1:ncol(preds)), function(k) {
    get_performance_metrics(predictions = preds[[k]][these_participants], labels = outcomes[these_participants],
                            weights = analysis_data$data[[toupper(args$weight)]][analysis_data$test_ids][these_participants],
  ...
  result_prefix <- paste0(
    ...
    "_model_", args$model_site, "_vaccine_", this_value
  )
  ...
  # variable importance
  set.seed(args$seed)
  est_vim <- get_vimp(phenorm_model = fit, 
                      preds = preds[these_participants, ],
                      newdata = analysis_data$data[analysis_data$test_ids, ][these_participants, ], 
                      silver_labels = toupper(silver_labels),
                      features = model_features,
                      utilization = toupper(analysis_data$utilization_variable), 
                      aggregate_labels = toupper(silver_labels),
                      outcomes = outcomes[these_participants],
                      measure = "permute")
  readr::write_csv(
    est_vim, file = paste0(
  ...
  raw_silver_perf <- lapply(as.list(seq_len(num_silvers)), function(k) {
    get_performance_metrics(predictions = analysis_data$data[analysis_data$test_ids, ][these_participants, ] %>% 
                              pull(silver_labels[k]), 
                            labels = outcomes[these_participants],
